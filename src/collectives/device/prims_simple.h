...
private:
  ...
  inline __device__ bool checkAbort(int &spins) {
    ...
    if (!(flags & Aborted) && spins == NCCL_SPINS_BEFORE_CHECK_ABORT) {
      flags |= atomicAdd_system((unsigned int *)shmem->comm.abortFlag, 0) ? Aborted : 0;
      spins = 0;
    }
  ...
  __device__ __forceinline__ void waitPeer(intptr_t dstIx, intptr_t remoteIx, int offset, int nelts) {
    ...
    if (((flags & (Recv*RoleWaitRecv)) && !noRecvWait) ||
      ...
      while (connStepCache + (isSendNotRecv ? NCCL_STEPS : 0) < step + StepPerSlice) {
        ...
        //if (spins == 0) printf("r=%d b=%d t=%d SPUN OUT got=%d want=%d\n", shmem->comm.rank, blockIdx.x, threadIdx.x, int(connStepCache + (isSendNotRecv ? NCCL_STEPS : 0)), int(step+StepPerSlice));
    ...
    if (flags & (Recv*RoleWaitRecv | Send*RoleWaitSend)) {
      ...
      void **ptrs = isSendNotRecv ? (shmem->groups[group].dsts + Dst)
                                  : (shmem->groups[group].srcs + Src);
    ...
    if (tid < nworkers && offset < nelem) {
      ...
      do {
        ...
        if (Src && (flags & (SrcBuf==Input ? RoleInput : RoleOutput)))
          shmem->groups[group].srcs[0] = userBuff + srcIx + offset;
        if (Dst && (flags & (DstBuf==Input ? RoleInput : RoleOutput)))
          shmem->groups[group].dsts[0] = userBuff + dstIx + offset;
        waitPeer<DirectRecv, DirectSend, Recv, Send, Src, Dst>(dstIx, remoteIx, offset, sliceSize);
        subBarrier();
        if (DirectRecv && shmem->groups[group].srcs[0] == shmem->groups[group].dsts[0]) {
          ...
          if (Send) {
            ...
            if (tid == 0) {
              NpKit::CollectGpuEvent(NPKIT_EVENT_PRIM_SIMPLE_REDUCE_OR_COPY_MULTI_ENTRY, sliceSize*sizeof(T), 0, __builtin_amdgcn_s_memrealtime(),
                  shmem->comm.npKitEventCollectContexts + npKitCtxIdx);
            }
#endif
            ...
            ReduceOrCopyMulti<Unroll, RedOp, T, 1, 1, 1, (1-Send)+MaxSend, 0>
              (tid, nworkers, nullptr, false,
               1, (T const**)shmem->groups[group].srcs,
               fan.nsend(), (T**)shmem->groups[group].dsts+1,
               sliceSize);
        ...
        } else if (DirectSend && !DirectRecv && SrcBuf != Input && shmem->groups[group].dsts[Dst] == nullptr) {
          ...
          ReduceOrCopyMulti<Unroll, RedOp, T, 1, 1, 1, 1, 0>
            (tid, nworkers, shmem->redOpArgs, postOp,
             Recv, (T const**)shmem->groups[group].srcs,
             Dst, (T**)shmem->groups[group].dsts,
             sliceSize);
          ...
          ReduceOrCopyMulti<Unroll, RedOp, T, Recv+Src, Recv*MaxRecv+Src, Send+Dst, Send*MaxSend+Dst, PreOpN>
            (tid, nworkers, shmem->redOpArgs, postOp,
             Recv*fan.nrecv()+Src, (T const**)shmem->groups[group].srcs,
             Send*fan.nsend()+Dst, (T**)shmem->groups[group].dsts,
             sliceSize);
    ...
    for (int slice=0; slice<SlicePerChunk; ++slice) {
      ...
      if (tid < nworkers) {
        if (Send) {
          ...
          if (flags & RoleInput) shmem->groups[group].srcs[0] = userBuff + inpIx + offset;
          if (tid == 0) shmem->groups[group].totalSendSize[slice] = 0; // Skip the threadfence
          ...
          for (int j=0; j<fan.nsend(); j++) {
            ...
            const T* src0 = (T*)shmem->groups[group].srcs[0] + peerOffset;
            int realPeerSize = min(realSize, totalElem-peerOffset);
            if (realPeerSize > 0 && shmem->groups[group].dsts[i] != nullptr) {
              ReduceOrCopyMulti<Unroll, RedOp, T, 1, 1, 1, 1, PreOpN>(tid, nworkers, shmem->redOpArgs, false, 1, &src0, 1, (T**)shmem->groups[group].dsts+i, realPeerSize);
              // Mark for threadfence at the end
              if (tid == 0) shmem->groups[group].totalSendSize[slice] += realPeerSize;
            }
          }
        } else if (Recv) {
          if (flags & RoleOutput) shmem->groups[group].dsts[0] = userBuff + outIx + offset;
          ...
          if (DirectRecv && shmem->groups[group].srcs[0] == shmem->groups[group].dsts[0]) {
            ...
            for (int j=0; j<fan.nrecv(); j++) {
              ...
              if (skip >= 0 && i >= skip) peerOffset += peerElem;
              T* dst0 = (T*)shmem->groups[group].dsts[0] + peerOffset;
              int realPeerSize = min(realSize, totalElem-peerOffset);
              if (realPeerSize > 0) ReduceOrCopyMulti<Unroll, RedOp, T, 1, 1, 1, 1, 0>(tid, nworkers, shmem->redOpArgs, postOp, 1, (const T**)shmem->groups[group].srcs+i, 1, &dst0, realPeerSize);
            }
          }
        }
      }
      ...
      if (Send && (flags & RolePostSend) && shmem->groups[group].totalSendSize[slice] > 0 && index == 0)
  ...
  __device__ __forceinline__ void loadRecvConn(ncclDevChannelPeer *peer, int connIndex, struct ncclWorkElem* e) {
    if (flags & (RoleWaitRecv|RolePostRecv)) {
      ...
      if (flags & RoleWaitRecv) {
        shmem->groups[group].recvConns[index] = conn; // WaitRecv role saves since that's who needs it in setDataPtrs()
  ...
  __device__ __forceinline__ void loadSendConn(ncclDevChannelPeer *peer, int connIndex, struct ncclWorkElem* e) {
    if (flags & (RoleWaitSend|RolePostSend)) {
      ...
      if (flags & RoleWaitSend) {
        shmem->groups[group].sendConns[index] = conn; // WaitSend role saves since that's who needs it in setDataPtrs()
        connStepPtr = conn->head;
        connStepCache = atomicAdd_system((unsigned long long *)connStepPtr, 0);
        flags |= (conn->offsFifo != nullptr) ? OffsFifoEnabled : 0;
        if (flags & OffsFifoEnabled)
 ...
 public:
  __device__ Primitives(
      struct ncclShmemData* shmemArg, int tid, int nthreads, int const *recvPeers, int const *sendPeers,
      void const *inputBuf, void *outputBuf, uint64_t redOpArg, uint32_t group=0, struct ncclWorkElem* e = nullptr
    ):
    shmem(shmemArg),
    tid(tid),
    stepSize(shmem->comm.buffSizes[NCCL_PROTO_SIMPLE]/NCCL_STEPS/sizeof(T)) {
    ...
    loadRecvConn(&shmem->channel.peers[peer], connIndex, e);
    loadSendConn(&shmem->channel.peers[peer], connIndex, e);

    setDataPtrs(inputBuf, outputBuf, redOpArg, (struct ncclWorkElemReg*)e);
  }

  __device__ ~Primitives() {
    // Ensure shmem->groups[].send/recvConns are available
    if (!(flags & ThreadsSynced))
      barrier();
    // Save steps for the next operation
    if (flags & (RolePostSend|RolePostRecv)) {
      auto *conns = (flags & RolePostSend) ? shmem->groups[group].sendConns : shmem->groups[group].recvConns;
  ...
  __device__ void setDataPtrs(void const *inputBuf, void *outputBuf, uint64_t redOpArg, struct ncclWorkElemReg* e) {
    if (flags & RoleInput) {
      userBuff = (T*)inputBuf;
      shmem->redOpArgs[0] = redOpArg;  // scaler for local input
    }
    ...
    if (Direct && recvProvider) {
      int spins = 0;
      void *volatile *slot = shmem->groups[group].recvConns[index]->ptrExchange;
    ...
    if (Direct && sendAcceptor) {
      int spins = 0;
      void *volatile *slot = shmem->groups[group].sendConns[index]->ptrExchange;
      void *ptr;
      while (true) {
    ...
    if (Direct && sendProvider) {
      int spins = 0;
      void *volatile *slot = shmem->groups[group].sendConns[index]->ptrExchange;
      volatile uint64_t* argSlot0 = shmem->groups[group].sendConns[index]->redOpArgExchange;
      volatile uint64_t* argSlot1 = shmem->groups[group].sendConns[index]->redOpArgExchange+1;
      // Wait for consumer to consume previous value before trampling it.
    ...
    if (Direct && recvAcceptor) {
      int spins = 0;
      void *volatile *slot = shmem->groups[group].recvConns[index]->ptrExchange;
      volatile uint64_t* argSlot0 = shmem->groups[group].recvConns[index]->redOpArgExchange;
      volatile uint64_t* argSlot1 = shmem->groups[group].recvConns[index]->redOpArgExchange+1;
      void *ptr;
      while (true) {
        ptr = (void *)atomicAdd_system((unsigned long long *) slot,0);
        if (ptr != nullptr || checkAbort(spins)) break;
      ...
      if (MaxSend != 0) { // reduce group rather than gather group
        ...
        shmem->redOpArgs[1+index] = ((arg1 & 0xffffffff)<<32) | (arg0 & 0xffffffff);
      }
