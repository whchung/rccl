...
struct RunWork<ncclFuncSendRecv, T, RedOp, NCCL_ALGO_RING, NCCL_PROTO_SIMPLE> {
  __device__ __forceinline__ void runSend(const int tid, const int nthreads, const int group, struct ncclWorkElemP2p* args, struct ncclShmemData* shmem) {
    ...
    if (args->peer == shmem->comm.rank) {
      ...
      Primitives<T, RedOp, FanAsymmetric<0, 1>, 0, Proto, 1> prims
        (shmem, tid, nthreads, nullptr, &peer, buff, nullptr, /*redOpArg(ignored)=*/0, group);
  ...
  __device__ __forceinline__ void runRecv(const int tid, const int nthreads, const int group, struct ncclWorkElemP2p* args, struct ncclShmemData* shmem) {
#if defined(ENABLE_NPKIT)
    bool isNpKitThread = (tid == 0);
    ...
    if (args->peer != shmem->comm.rank) {
      ...
      Primitives<T, RedOp, FanAsymmetric<1, 0>, 0, Proto, 1> prims
        (shmem, tid, nthreads, &peer, nullptr, nullptr, buff, /*redOpArg(ignored)=*/0, group);
  ...
  __device__ __forceinline__ void run(ncclWork *work, struct ncclShmemData* shmem) {
    ...
    if ((group%2) == 0) {
      runRecv(tid, nthreads, group, args, shmem);
    } else {
      runSend(tid, nthreads, group, args, shmem);
    }
  }
};
